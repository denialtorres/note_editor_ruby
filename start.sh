...
if [ -n "$1" ]; then
  export NOTE_FILE="$(cd "$(dirname "$1")" && pwd)/$(basename "$1")"
...
for _ in $(seq 1 50); do
  curl -s -o /dev/null "$URL/api/initial" && break
  sleep 0.1
done

open -na "Google Chrome" --args --app="$URL" --window-size=1000,900
